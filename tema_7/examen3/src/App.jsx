                                        ...
                                        if (filtro !== "") {
                                            if (value.descATC.toLowerCase().includes(filtro.toLowerCase())) {
                                                return <option key={index}
                                                               value={value.codATC + "|" + value.descATC}>{value.codATC + "|" + value.descATC}</option>
                                            }
                                        } else {
                                            return <option key={index}
                                                           value={value.codATC + "|" + value.descATC}>{value.codATC + "|" + value.descATC}</option>
...
class Filter extends Component {
    ...
    add(datos) {
        ...
        if (datos[2] === "EXCLUIR") {
            ...
            this.setState({rxenmascarar: this.state.rxenmascarar + ", " + datos[0]});
        } else {
            if (this.state.rxseleccionar === "") {
                this.setState({rxseleccionar: datos[0]});
                return;
            }
            this.setState({rxseleccionar: this.state.rxseleccionar + ", " + datos[0]});
        }
    }
        ...
        return (
            <>
                                    ...
                                    <Col>
                                        <Alert color="info">
                                            ...
                                            <Button onClick={() => {
                                                ...
                                                this.toggleModal()
                                            }} color="info">Add</Button>{" "}<Button color="info"
                                                                                     onClick={() => {
                ...
                <VentanaModalDiccionario diccionario={this.state.diccionario}
                                         ...
                                         titulo={"ADD " + this.state.diccionario} excluir={this.state.excluir}/>
